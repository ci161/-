...
class AutoYoulaSpider(scrapy.Spider):
    ...
    _css_selectors = {
        'brands': 'div.TransportMainFilters_brandList__2tIkv '
                  '.ColumnItemList_item__32nYI a.blackLink',
        'pagination': 'a.Pagination_button__u1e7D',
    ...
    data_query = {
        ...
        'photos': lambda resp: [
            itm.attrib.get('src') for itm in resp.css('fiure.PhotoGallery_photo__36e_r img')
        ],
        'characteristics': lambda resp: [
            {
                ...
                or itm.css('.AdvertSpecs_data__xK2Qx::text').extract_first()
            }
    ...
    def car_parse(self, response):
        ...
        for key, selector in self.data_query.items():
            try:
                ...
                continue
        self.db_client['localhost:27017'][self.name].insert_one(data)
